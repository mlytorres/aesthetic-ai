...
export default function AppSidebarLayout({
    ...
    const { showWarning, remainingSeconds, extendSession, logout } =
        useSessionTimeout();
    const { impersonating, tenantId } = usePage().props as unknown as {
        impersonating: { as: string } | null;
    ...
    return (
        <>
            {impersonating && (
                <div className="fixed top-0 right-0 left-0 z-50 flex items-center justify-between bg-amber-400 px-4 py-2 shadow-md">
                    <span className="text-sm font-semibold text-black">
                        👤 Impersonating <strong>{impersonating.as}</strong> —
                        changes you make affect this tenant's data.
                    </span>
                    <button
                        type="button"
                        onClick={stopImpersonating}
                        className="rounded bg-black/10 px-3 py-1 text-xs font-bold text-black transition-colors hover:bg-black/20"
                    >
                        Stop Impersonating
                    </button>
                </div>
                        ...
                        >
                            <DialogHeader>
                                ...
                                <DialogDescription className="text-muted-foreground">
                                    For HIPAA compliance, your session will
                                    automatically end after 30 minutes of
                                    inactivity. You will be logged out in{' '}
