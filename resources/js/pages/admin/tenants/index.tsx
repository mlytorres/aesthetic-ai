...
export default function TenantsIndex({ tenants }: Props) {
    const handleToggle = (tenant: Tenant) => {
        if (tenant.active) {
            if (
                !confirm(
                    `Deactivate "${tenant.name}"? Their staff will lose access.`,
                )
            ) {
                return;
            }
            ...
            <div className="space-y-8">
                <div className="flex items-center justify-between">
                    ...
                    <Link href="/admin/tenants/create">
                        <Button className="gap-2 bg-[#0E9E8E] text-[#0A0A0F] hover:bg-[#0E9E8E]/90">
                ...
                <div className="grid grid-cols-3 gap-4">
                    {[
                        ...
                        >
                            ...
                            <p
                                className={`mt-1 text-3xl font-bold ${stat.color}`}
                            >
                                {stat.value}
                            </p>
                        </div>
                            ...
                            <table className="w-full">
                                ...
                                <tbody>
                                    {tenants.map((tenant) => (
                                        <tr
                                            key={tenant.id}
                                            className="border-b border-sidebar-border/30 transition-colors hover:bg-background/50"
                                        >
                                            <td className="px-4 py-3">
                                                <Link
                                                    href={`/admin/tenants/${tenant.id}`}
                                                    className="font-medium text-foreground transition-colors hover:text-[#0E9E8E]"
                                                >
                                                    {tenant.name}
                                                </Link>
                                            </td>
